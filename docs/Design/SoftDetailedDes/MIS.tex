...
by following steps. 
\end{itemize}
\textit{\# Get user input and set it in the FlockingParameter object}
...
\textit{\# Save the parameters to a JSON file}
...
\end{itemize}
\textit{\# Get the flocking parameters from the user input path}

parameters :=  GetSimulateParam()

method := GetMethod()

\textit{\# Calculate the metrics of flocking in the simulation}
...
\textit{\# Generate the animation and display}
...
\begin{itemize}
\item transition: Read the animation information from the paht users input from 
...
\textit{\# Draw the line charts and display}
...
\texttt{CalObsAForce} & \texttt{p}: $\mathbb{R}^3$, \texttt{obs}: Obstacle[] & $\mathbb{R}^3$ & - \\
...
\noindent EEulerMethod(i, ps, vs, info, goal):
\begin{itemize}
\item output: \\
...
\textit{\# solving a = CalForce(i, ps, vs, info, goal) by Force Module}
\begin{itemize}
\item output: \\
$k_1 := \begin{bmatrix} a_1 \\ v \end{bmatrix}$ \\
$k_2 := k_1 + \begin{bmatrix} a_2 \\ v + \frac{t}{2}k_{1} \end{bmatrix}$ \\
...
\section{MIS of Obstacle Module} \label{mObs}
...
\item nInput: the UI input object used to read the value of number of ageng.
...
\item exception: 
\begin{itemize}
    \item ErrorObject, if for any paht, there is no such file or the 
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\texttt{DrawChart} & \texttt{matrics}: FlockingStatus[] & - & - \\
\hline
\end{tabular}
\end{center}
