...
\begin{tabular}{l l} 
  ...
  \midrule 
  T & A generic data type that the module will operate on.\\
  \bottomrule
\end{tabular}\\
...
\begin{itemize}
\item $screen$: A display device used to present error message or file's 
saving path to the user. 
\end{itemize}
...
initial positions at the FlockingStatus.starts, save it using File I/O Module 
by following steps, and show the path in the $screen$. 
\end{itemize}
...
\begin{itemize}
\item transition: Read the input parameters from Input Reader Module, calculate 
the process of simulation using Dynamics Module, save the results using 
File I/O Module by following steps, and show the path in the $screen$. 
\end{itemize}
...
\begin{itemize}
\item \textit{\# Used to identify the name of variable that caused the error when generating error messages.}

$\mathit{InputType} := \{KA, KS, KC, KO, KG, T, N, R, P\}$
\end{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\texttt{KVerify} & \texttt{input}: str, \texttt{InputType}: type & - & MissingK, ErrorK \\
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\texttt{CalForce} & \texttt{i}: $\mathbb{R}$, \texttt{ps}: sequence of $\mathbb{R}^3$, 
\texttt{vs}: sequence of $\mathbb{R}^3$, \texttt{info}: FlockingParameter, 
\texttt{goal}: $\mathbb{R}^3$ & $\mathbb{R}^3$ & - \\
\texttt{CalAlignForce} & \texttt{i}: $\mathbb{R}$, \texttt{vs}: sequence of $\mathbb{R}^3$ & $\mathbb{R}^3$ & - \\
\texttt{CalCoheForce} & \texttt{i}: $\mathbb{R}$, \texttt{ps}: sequence of $\mathbb{R}^3$ & $\mathbb{R}^3$ & - \\
\texttt{CalSepaForce} & \texttt{i}: $\mathbb{R}$, \texttt{ps}: sequence of $\mathbb{R}^3$ & $\mathbb{R}^3$ & - \\
...
\begin{itemize}
\item output: out := $\mathit{info}.ka \cdot CalAlignForce(i, vs) + \mathit{info}.kc \cdot CalCoheForce(i, ps) + 
\mathit{info}.ks \cdot CalSepaForce(i, ps) + \mathit{info}.ko \cdot CalObsAForce(ps[i], info.obs) + 
\mathit{info}.kg \cdot CalGoalForce(ps[i], vs[i], goal) $. 
\end{itemize}

\noindent CalAlignForce(i, vs):
\begin{itemize}
\item output: out := $FAlign=\frac{1}{|N_i|}\sum_{j\in N_i}vs_j-vs_i$. 
\end{itemize}

\noindent CalCoheForce(i, ps):
\begin{itemize}
\item output: out := $FCohe=\frac{1}{|N_i|}\sum_{j\in N_i}ps_j-ps_i$. 
\end{itemize}

\noindent CalSepaForce(i, ps):
\begin{itemize}
\item output: out := $FSep=\sum_{j\in N_i}\frac{ps_i-ps_j}{\|ps_i-ps_j\|^2}$. 
\end{itemize}

\noindent CalObsAForce(p, obs):
\begin{itemize}
\item output: out := $FObs=\sum_{k\in obs}\frac{p_i-obs_k}{\|p_i-obs_k\|^2}$. 
\end{itemize}

\noindent CalGoalForce(p, v, goal):
\begin{itemize}
\item output: out := $FGoal=vMax\cdot\frac{g-p}{\|g-p\|}-v$. 
\end{itemize}
...
\begin{itemize}
\item \texttt{SimuRes}: the simulation result.
\begin{itemize}
  \item \texttt{velos}: sequence of $\mathbb{R}^3$, a set of velocitie for all 
  agents at this time.
  \item \texttt{positions}: sequence of $\mathbb{R}$, a set of position for all 
  agents at this time.
\end{itemize}
\end{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\texttt{EEulerMethod} & \texttt{ps}: sequence of $\mathbb{R}^3$, 
\texttt{vs}: sequence of $\mathbb{R}^3$, \texttt{info}: FlockingParameter, 
\texttt{goal}: $\mathbb{R}^3$ & SimuRes & - \\
\texttt{RK4Method} & \texttt{ps}: sequence of $\mathbb{R}^3$, 
\texttt{vs}: sequence of $\mathbb{R}^3$, \texttt{info}: FlockingParameter, 
\texttt{goal}: $\mathbb{R}^3$ SimuRes & - \\
\texttt{SIEulerMethod} & \texttt{ps}: sequence of $\mathbb{R}^3$, 
\texttt{vs}: sequence of $\mathbb{R}^3$, \texttt{info}: FlockingParameter, 
\texttt{goal}: $\mathbb{R}^3$& SimuRes & - \\
\hline
\end{tabular}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\texttt{CalMeanSpeed} & \texttt{vs}: sequence of $\mathbb{R}^3$ & $\mathbb{R}$ & - \\
\texttt{CalAlignMetrics} & \texttt{vs}: sequence of $\mathbb{R}^3$ & $\mathbb{R}$ & - \\
\texttt{CalCenter} & \texttt{ps}: sequence of $\mathbb{R}^3$ & $\mathbb{R}^3$ & - \\
\texttt{CalCohesion} & \texttt{ps}: sequence of $\mathbb{R}^3$, \texttt{center}: $\mathbb{R}^3$ & $\mathbb{R}$ & - \\
\hline
\end{tabular}
\end{center}
...
\textit{\# Compute the average magnitude of the velocities of all agents.}
\begin{itemize}
\item output: out := $\frac{1}{|vs|} \sum_{i=1}^{|vs|} \left\lVert vs[i] \right\rVert$. 
\end{itemize}

\noindent CalAlignMetrics(vs):
\textit{\# Compute the average of the sum of the normalized velocities of all agents.}
\begin{itemize}
\item output: out := $\frac{1}{|vs|}\left\|\sum_{i=1}^{|vs|}\frac{vs[i]}{\|vs[i]\|}\right\|$. 
\end{itemize}

\noindent CalCenter(ps):
\begin{itemize}
\item output: out := $\frac{1}{|ps|} \sum_{i=1}^{|ps|} ps[i]$. 
\end{itemize}

\noindent CalCohesion(ps, center):
\begin{itemize}
\item output: out := $\frac{1}{|ps|} \sum_{i=1}^{|ps|} \left\| ps[i] - \text{center} \right\|$. 
\end{itemize}
...
\end{itemize}
\textit{\# Get the FlockingParameter object from InputReader by MainController}
...
\noindent ODEChoose():
\begin{itemize}
\item input: \texttt{method}: str, \texttt{i}: $\mathbb{R}$, \texttt{ps}: sequence of $\mathbb{R}^3$, 
\texttt{vs}: sequence of $\mathbb{R}^3$, \texttt{info}: FlockingParameter. 
\item output: sequence of $\mathbb{R}^3$.
\item exception: exc := ErrorMethod, if the ODE method is not defined.
...
\begin{itemize}
\item \texttt{t}: $\mathbb{R}$, the length of time step. 
\item \texttt{agents}: sequence of (sequence of $\mathbb{R}^3$), the position of agents. 
\item \texttt{obs}: array of Obs, the information of obstacles;
\end{itemize}
...
\begin{itemize}
\item \texttt{t}: $\mathbb{R}$, the length of time step. 
\item \texttt{meanSpeed}: sequence of $\mathbb{R}$, the list of mean speed at every time step. 
\item \texttt{alignMetric}: sequence of $\mathbb{R}$, the list of align at every time step.
\item \texttt{cohesion}: sequence of $\mathbb{R}$, the list of cohesion at every time step.
\end{itemize}
...
\texttt{GetFlockStatus} & - & sequence of FlockingStatus & ErrorObject \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
\item output: \texttt{out} := sequence of AnimationStatus, a list of AnimationStatus object 
...
\begin{itemize}
\item output: \texttt{out} := sequence of FlockingStatus, a list of FlockingStatus object 
read from the user-specified paths.
\item exception: 
\begin{itemize}
    \item ErrorObject, if for any path, there is no such file or the 
...
\subsubsection{Environment Variables}
\begin{itemize}
\item $file$: the contents of file at specific path. 
\end{itemize}
...
\begin{itemize}
\item transition: write the contents into $file$ at path.
\item output: None.
\end{itemize}
...
\subsubsection{Environment Variables}
\begin{itemize}
\item $screen$: A display device used to present animation to the user. 
\end{itemize}
...
\begin{itemize}
\item transition: display the animation in the $screen$.
\item output: None.
\end{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\texttt{DrawChart} & \texttt{metrics}: sequence of FlockingStatus & - & - \\
\hline
\end{tabular}
\end{center}
...
\subsubsection{Environment Variables}
\begin{itemize}
\item $screen$: A display device used to present charts to the user. 
\end{itemize}
...
\begin{itemize}
\item transition: display line charts in the $screen$.
\item output: None.
\end{itemize}
